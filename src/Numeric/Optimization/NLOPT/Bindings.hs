...
data Algorithm
    ...
    | {- | DIviding RECTangles, original FORTRAN
      implementation
      -}
      GN_ORIG_DIRECT
    | {- | DIviding RECTangles,
    ...
    | -- | Limited-memory BFGS
      LD_LBFGS
    | {- | PRincipal AXIS gradient-free local
      optimization
      -}
      LN_PRAXIS
    | {- | Shifted limited-memory
    ...
    | -- | Truncated Newton's method
      LD_TNEWTON
    | {- | Truncated Newton's method with
      automatic restarting
      -}
      LD_TNEWTON_RESTART
    | {- | Preconditioned truncated Newton's
      method
      -}
      LD_TNEWTON_PRECOND
    | {- | Preconditioned truncated Newton's
    ...
    | {- | Controlled Random Search with
      Local Mutation
      -}
      GN_CRS2_LM
    | {- | Original Multi-Level
      Single-Linkage
      -}
      GN_MLSL
    | {- | Original Multi-Level
      ...
      -}
      GD_MLSL
    | {- | Multi-Level Single-Linkage with
    ...
    | -- | Powell's NEWUOA algorithm
      LN_NEWUOA
    | {- | Powell's NEWUOA algorithm with
      bounds by SGJ
      -}
      LN_NEWUOA_BOUND
    | {- | Nelder-Mead Simplex gradient-free
      ...
      -}
      LN_NELDERMEAD
    | {- | NLOPT implementation of Rowan's
      Subplex algorithm
      -}
      LN_SBPLX
    ...
    | {- | AUGmented LAGrangian,
      ...
      -}
      LD_AUGLAG
    | {- | AUGmented LAGrangian with penalty
...
set_local_optimizer p s =
    withOpt p $ \primary -> withOpt s $ (fmap parseEnum . nlopt_set_local_optimizer primary)
